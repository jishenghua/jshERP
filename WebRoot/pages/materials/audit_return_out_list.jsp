...
<html>
  	...
  	<body>
		...
		<!-- 数据显示table -->
		<div id = "tablePanel"	class="easyui-panel" style="padding:1px;top:300px;" title="退还出库审核列表" iconCls="icon-list" collapsible="true" closable="false">
		...
		<script type="text/javascript">
			...
			{
				$.messager.confirm('删除确认','确定要删除此退还出库审核信息吗？',function(r)
			 	{
                    if (r)
                    {
						$.ajax({
							...
							success: function (tipInfo)
							{
								...
								if(msg == '成功')
								{
									...
									$.messager.alert('删除提示','删除退还出库审核信息失败，请稍后再试！','error');
							},
							//此处添加错误处理
				    		error:function()
				    		{
				    			$.messager.alert('删除提示','删除退还出库审核信息异常，请稍后再试！','error');
								return;
							}
						});			
                    }
			...
			function batDeleteDepotHead()
			{
				...
				if(row.length > 0)
				{
					$.messager.confirm('删除确认','确定要删除选中的' + row.length + '条退还出库审核信息吗？',function(r)
				 	{
	                    if (r)
	                    {
	                    	var ids = "";
			...
			function addDepotHead()
			{
				...
				$('#depotHeadDlg').dialog('open').dialog('setTitle','<img src="<%=path%>/js/easyui-1.3.5/themes/icons/edit_add.png"/>&nbsp;增加退还出库审核信息');
				...
				click:function()
				{
					...
					else 
					{
						$.ajax({
				    		...
				    		error:function()
				    		{
				    			$.messager.alert('提示','保存退还出库审核信息异常，请稍后再试！','error');
								return;
							}
						});	
					}
	        ...
	        function editDepotHead(depotHeadTotalInfo)
	        {
                ...
                $('#depotHeadDlg').dialog('open').dialog('setTitle','<img src="<%=path%>/js/easyui-1.3.5/themes/icons/pencil.png"/>&nbsp;编辑退还出库审核信息');
                $(".window-mask").css({ width: webW ,height: webH});
	        ...
	        function showDepotHead(depotHeadTotalInfo)
	        {
                ...
                $('#depotHeadDlgShow').dialog('open').dialog('setTitle','<img src="<%=path%>/js/easyui-1.3.5/themes/icons/list.png"/>&nbsp;查看退还出库审核信息'+audit);
                $(".window-mask").css({ width: webW ,height: webH});
			...
			function submitDepotHead()
			{
	            ...
	            if(row.length > 0)
				{
					$.messager.confirm('审批确认','确定要提交选中的' + row.length + '条退还出库审核信息吗？',function(r)
				 	{
	                    if (r)
	                    {
	                    	var ids = "";

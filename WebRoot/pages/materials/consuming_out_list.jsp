...
<html>
  	...
  	<body>
		...
		<!-- 数据显示table -->
		<div id = "tablePanel"	class="easyui-panel" style="padding:1px;top:300px;" title="领用出库列表" iconCls="icon-list" collapsible="true" closable="false">
	    ...
	    <div id="depotHeadDlg" class="easyui-dialog" style="width:850px;padding:10px 20px;top:20px"
	        ...
	        <form id="depotHeadFM" method="post"  novalidate>
	            ...
	            <td>单元：</td>
				<td style="padding:5px">
				<select name="BuildingId" id="BuildingId" style="width:120px;"></select>
				</td>
	            <td>备注：</td>
	            <td style="padding:5px">
		...
		<script type="text/javascript">
			...
			//初始化系统基础信息
			function initSystemData_building(ProjectSearch)
			{
				$.ajax({
					type:"post",
					url: "<%=path%>/building/getBasicData.action?ProjectId="+ProjectSearch,
					//设置为同步
					async:false,
					dataType: "json",
					success: function (systemInfo)
					{
						buildingList = systemInfo.showModel.map.buildingList;
						var msgTip = systemInfo.showModel.msgTip;
						if(msgTip == "exceptoin")
						{
							$.messager.alert('提示','查找系统基础信息异常,请与管理员联系！','error');
			...
			//初始化页面选项卡
			function initSelectInfo_building()
			{
				var options1 = "";
				if(buildingList !=null)
				{
					for(var i = 0 ;i < buildingList.length;i ++)
					{
						...
						options1 += '<option value="' + building.id + '">' + building.name + '</option>';	
					}
					$("#BuildingId").empty().append(options1);
				}
			}
			...
			$("#ProjectId").change(
					function(){
						...
						if(ProjectId!='')
						{
						    ...
						    //获取单元
							initSystemData_building(ProjectId);
							initSelectInfo_building();
						}
					}
				);
			          ...
			          { title: '操作',field: 'op',align:"center",width:180,formatter:function(value,rec)
			         	{
							...
							+ 'AaBb' + rec.WareHousePersonName+ 'AaBb' + rec.BuildingId+ 'AaBb' + rec.BuildingName
			...
			{
				$.messager.confirm('删除确认','确定要删除此领用出库信息吗？',function(r)
			 	{
                    if (r)
                    {
						$.ajax({
							...
							success: function (tipInfo)
							{
								...
								if(msg == '成功')
								{
									...
									$.messager.alert('删除提示','删除领用出库信息失败，请稍后再试！','error');
							},
							//此处添加错误处理
				    		error:function()
				    		{
				    			$.messager.alert('删除提示','删除领用出库信息异常，请稍后再试！','error');
								return;
							}
						});			
                    }
			...
			function batDeleteDepotHead()
			{
				...
				if(row.length > 0)
				{
					$.messager.confirm('删除确认','确定要删除选中的' + row.length + '条领用出库信息吗？',function(r)
				 	{
	                    if (r)
	                    {
	                    	var ids = "";
			...
			function addDepotHead()
			{
				...
				$('#depotHeadDlg').dialog('open').dialog('setTitle','<img src="<%=path%>/js/easyui-1.3.5/themes/icons/edit_add.png"/>&nbsp;增加领用出库信息');
					...
					else 
					{
						$.ajax({
							...
							data: ({
								...
								BuildingId: $.trim($("#BuildingId").val()),
				    		...
				    		error:function()
				    		{
				    			$.messager.alert('提示','保存领用出库信息异常，请稍后再试！','error');
								return;
							}
						});	
					}
	        ...
	        function editDepotHead(depotHeadTotalInfo)
	        {
	            ...
	            $("#BuildingId").val(depotHeadInfo[14]);
	            
	            //orgDepotHead = depotHeadInfo[1];
                $('#depotHeadDlg').dialog('open').dialog('setTitle','<img src="<%=path%>/js/easyui-1.3.5/themes/icons/pencil.png"/>&nbsp;编辑领用出库信息');
                $(".window-mask").css({ width: webW ,height: webH});
	        ...
	        function showDepotHead(depotHeadTotalInfo)
	        {
	            ...
	            $("#BuildingIdShow").text(depotHeadInfo[15]);
                ...
                $('#depotHeadDlgShow').dialog('open').dialog('setTitle','<img src="<%=path%>/js/easyui-1.3.5/themes/icons/list.png"/>&nbsp;查看领用出库信息'+audit);
                $(".window-mask").css({ width: webW ,height: webH});
			...
			function submitDepotHead()
			{
	            ...
	            if(row.length > 0)
				{
					$.messager.confirm('提交审核确认','确定要提交选中的' + row.length + '条领用出库信息吗？',function(r)
				 	{
	                    if (r)
	                    {
	                    	var ids = "";
			...
			function cancelDepotHead()
			{
	            ...
	            if(row.length > 0)
				{
					$.messager.confirm('废弃确认','确定要废弃选中的' + row.length + '条领用出库信息吗？',function(r)
				 	{
	                    if (r)
	                    {
	                    	var ids = "";
